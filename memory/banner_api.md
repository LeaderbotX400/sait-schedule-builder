...
`GET https://sait-sust-prd-prd1-ban-ss-ssag2.sait.ca/BannerGeneralSsb/ssb/PersonalInformationDetails/getBannerId`
Returns a JSON object with the student's Banner ID, e.g. `{ bannerId: number }`. This is used in subsequent API calls to fetch student-specific data.
